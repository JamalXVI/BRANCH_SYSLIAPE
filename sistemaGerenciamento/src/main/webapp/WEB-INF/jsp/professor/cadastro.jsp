...
<div class="row col-xs-12" role="main">
	...
	<div class="col-xs-12">
		...
		<div class="col-xs-6 col-md-4">
			<input class="form-control" placeholder="Filtrar" id="pesquisarProf"
				name="pesquisarProf" />
		</div>
...
<t:rodape>
	...
	<script type="text/javascript">
		...
		var fazerListaProfessores = function() {
			$("#listaprofessores").find("#panel_paginacao_tabela").remove().end();
			...
			conteudoTabelaProf = [];
			$(professores).each(
					function(i, professor) {
						//Verificar se o Professor está no Filtro
						if (professor.pessoa.nome.toLowerCase().indexOf(
								filtroUsr) >= 0
								|| professor.pessoa.sobrenome.toLowerCase()
										.indexOf(filtroUsr) >= 0) {
		...
		var adicionarProfessorTabela = function(professor, i) {
			...
			if (professor.pessoa.telefones.length > 0) {
				...
				telefone = "(" + tel.ddd + ")" + tel.numero + " - Op:"
						+ tel.operadora;
			}
			;
			...
			if (temPermissao) {
				acoesProfessores = criarTextoAcao("editarProfessor",
						"fa fa-pencil", i)
						+ criarTextoAcao("excluirProfessor", "fa fa-trash",
								professor.codigo)
			}
			//Adicionar professor na tabela correta.
			$("#corpotabelaListaProfessores")
					.append(
							"<tr><td><img id='fotoPerfilprofessor"
			+professor.idPes+"' class='imagemFotoprofessor'"+
			"></img></td><td class='my-table-label-body'>"
									+ professor.pessoa.nome
									+ " "
									+ professor.pessoa.sobrenome
									+ "</td>"
									+ "<td class='my-table-label-body' style='text-transform:none;'>"
									+ professor.pessoa.email
									//				'my-table-label-body'
									+ "</td><td class='my-table-label-body'>"
									+ telefone
									+ "</td><td style='display:none;' class='professoresis'>"
									+ professor.codigo
									+ "</td><td class='my-table-label-body'>"
									+ criarTextoAcao("verProfessor",
											"fa fa-search", i)
		...
		var posicionarImagemFotoPerfil = function(professor) {
			$("#fotoPerfilprofessor" + professor.idPes).attr("src",
					"data:image/png;base64," + fotos_perfil[professor.idPes]);
		}
		...
		var editarProfessor = function(indice) {
			...
			$("#dataNascimento").val(
					dtn.year + "-" + retornarComZero(dtn.month) + "-"
							+ retornarComZero(dtn.day));
			$("#codigoProfessor").val(professor.codigo);
			removerTelefones();
			$(professor.pessoa.telefones).each(
					function(i, telefone) {
						adicionar_telefones(telefone.ddd, telefone.numero,
								telefone.operadora);
					});
							...
							$("#nome")
									.rules(
											"add",
											{
												...
												minlength : 2,
												messages : {
													required : iniMensagemAlerta
															+ "Campo Obrigatório"
															+ fimMensagemAlerta,
													minlength : jQuery.validator
															.format(iniMensagemAlerta
																	+ "Por Favor insira ao menos {0} caracteres."
																	+ fimMensagemAlerta)
												}
											});
							$("#sobrenome")
											...
											"add",
											{
												...
												messages : {
													...
													maxlength : jQuery.validator
															.format(iniMensagemAlerta
																	+ "Por Favor insira até {0} caracteres."
																	+ fimMensagemAlerta)
												}
											});
							$("#senha")
	...
	<script type="text/javascript">
		var verProfessor = function(indice) {
			...
			var dtn = professor.pessoa.datanascimento;
			$("#dataNascimentoVer").val(
					retornarComZero(dtn.day) + "/" + retornarComZero(dtn.month)
							+ "/" + dtn.year);
			$("#codigoVer").val(professor.codigo);
			$(professor.pessoa.telefones).each(
					function(i, telefone) {
						$("#verTelefones").append(
								"<div><label>Telefone " + i
