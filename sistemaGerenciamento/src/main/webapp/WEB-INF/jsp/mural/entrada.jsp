...
<c:import url="/WEB-INF/jsp/scripts/PaginacaoMuralComum.jsp" />
	<script type="text/javascript">
		var receberMensagens = function() {
			return $.ajax({
				type : "post",
				url : "${linkTo[MuralController].listarMensagens() }",
...
<c:import url="/WEB-INF/jsp/scripts/ExcluirMuralComum.jsp" />
<script type="text/javascript">
	var apagarMensagens = function(idMur) {
		return $.ajax({
			type : "post",
			url : "${linkTo[MuralController].apagarMensagem() }",
			data: {"idMur" : idMur},
			dataType : "json", // Isso diz que você espera um JSON do servidor
			beforeSend : function(xhr, settings) {
