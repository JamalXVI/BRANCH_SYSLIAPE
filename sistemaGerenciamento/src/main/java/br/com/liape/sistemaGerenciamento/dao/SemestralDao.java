...
public class SemestralDao extends PersistenceJDBC<Semestral> {
	...
	private static String SQL_LISTAR_HORA = "SELECT * FROM SEMESTRAL WHERE HORA_INICIO_SEM >= ? AND"
			+ " HORA_INICIO_SEM <= ? AND ID_RES = ? AND ((DAYOFWEEK(?) + 5) % 7) = DIA_SEMANA_SEM AND ATIVO_SEM = TRUE";
